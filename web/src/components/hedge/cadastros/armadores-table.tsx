...
import { ArmadorData, ItemTabelaArmadorData } from "@/lib/hedge-data";
import {
  createArmador,
  updateArmador,
  deleteArmador,
  createItemTabelaArmador,
  updateItemTabelaArmador,
  deleteItemTabelaArmador,
} from "@/app/(dashboard)/hedge/mesa-operacao/armadores/actions";
import { Plus, Pencil, Trash2, Check, X, ChevronDown } from "lucide-react";

function ItemRow({ item }: { item: ItemTabelaArmadorData }) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      await updateItemTabelaArmador(item.id, descricao.trim(), Number(preco.replace(",", ".")) || 0);
      setEditing(false);
      router.refresh();
    });
  }
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      await deleteItemTabelaArmador(item.id);
      router.refresh();
    });
  }
...
function NovoItemForm({ armadorId }: { armadorId: string }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  ...
  function handleCreate() {
    if (!descricao.trim()) return;
    startTransition(async () => {
      await createItemTabelaArmador(armadorId, descricao.trim(), Number(preco.replace(",", ".")) || 0);
      setDescricao("");
      setPreco("");
      router.refresh();
    });
  ...
  return (
    <div className="flex items-center gap-2">
      <Input
        ...
        placeholder="Ex: THC 20', ISPS, Lacre"
...
export function ArmadoresTable({ armadores }: { armadores: ArmadorData[] }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  ...
  function handleCreate() {
    ...
    startTransition(async () => {
      try {
        await createArmador(newName.trim());
        setNewName("");
        router.refresh();
      } catch {
  ...
  function startEdit(a: ArmadorData) {
    setEditingId(a.id);
  ...
  function handleUpdate() {
    ...
    startTransition(async () => {
      try {
        await updateArmador(editingId, editingName.trim());
        setEditingId(null);
        router.refresh();
      } catch {
  ...
  function handleDelete(id: string) {
    if (
      !window.confirm(
        "Excluir este armador? Contratos que já usam esse armador mantêm o registro, mas ele deixa de aparecer nas opções."
      )
    )
      return;
    setError(null);
    startTransition(async () => {
      try {
        await deleteArmador(id);
        router.refresh();
      } catch {
        setError("Não foi possível excluir: este armador está em uso em algum contrato.");
      }
    });
  }
      ...
      <CardContent className="space-y-3 p-4">
        <div className="flex gap-2">
          <Input
            ...
            onKeyDown={(e) => e.key === "Enter" && handleCreate()}
            placeholder="Ex: Maersk, MSC, Hapag-Lloyd"
          />
        ...
        <div className="divide-y divide-border rounded-lg border border-border">
          {armadores.length === 0 && (
            <p className="p-4 text-center text-sm text-muted">Nenhum armador cadastrado ainda.</p>
          )}
          {armadores.map((a) => {
                  ...
                  {editingId === a.id ? (
                    ...
                    <>
                      ...
                      >
                        ...
                        />
                        {a.name}
                        <span className="text-xs text-muted">({a.itens.length} itens)</span>
                      </button>
                      <button onClick={() => startEdit(a)} className="rounded-md p-1.5 text-muted hover:bg-border/60 hover:text-foreground" title="Editar">
                        <Pencil size={14} />
                      </button>
                      <button
