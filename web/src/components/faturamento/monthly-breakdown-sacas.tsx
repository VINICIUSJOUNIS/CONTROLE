...
export function MonthlyBreakdownSacas({ year, rows }: { year: string; rows: MonthRow[] }) {
        ...
        <table className="w-full whitespace-nowrap text-sm print:text-[10px]">
          <thead>
            <tr className="border-b border-border text-left text-xs text-muted print:text-[9px]">
              ...
              <th className="px-4 py-2.5 font-medium print:px-2 print:py-1">Volume Geral (sacas)</th>
              <th className="px-4 py-2.5 font-medium print:px-2 print:py-1">Mercado Interno (sacas)</th>
              <th className="px-4 py-2.5 font-medium print:px-2 print:py-1">% Interno</th>
              <th className="px-4 py-2.5 font-medium print:px-2 print:py-1">Mercado Externo (sacas)</th>
            ...
            {rows.map((r) => (
              <tr key={r.label} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-2.5 print:px-2 print:py-0.5">
                  {r.geral.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                </td>
                <td className="px-4 py-2.5 print:px-2 print:py-0.5">
                  {r.interno.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                </td>
                <td className="px-4 py-2.5 print:px-2 print:py-0.5">{formatPercent(r.pctInterno, 1)}</td>
                <td className="px-4 py-2.5 print:px-2 print:py-0.5">
                  {r.externo.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                </td>
